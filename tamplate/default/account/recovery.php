...
$result3 = mysqli_query(table(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME), "SELECT * FROM  $name WHERE ($SelectFilter3) AND ($SelectFilter2) ");
while ($tettabl3 = mysqli_fetch_assoc($result3)) {
    $bases3[] = $tettabl3;
}


if (empty($bases3[0]) != true) {
    $exist = "yes";
} else {
    $exist = "no";
}
...
if ($exist == "yes") {
    $result = mysqli_query(table(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME), "SELECT * FROM  $name WHERE ($SelectFilter2) AND ($SelectFilter3)");
    ...
    $mail->Port = "465";
    //Set gmail username
    $mail->Username = "no-reply@newworldinfo.info";
    //Set gmail password
    $mail->Password = "090399s090399S";
    //Email subject
    $mail->Subject = "Восстановление доступа";
    //Set sender email
    $mail->setFrom('no-reply@newworldinfo.info');
    //Enable HTML
    $mail->isHTML(true);
    //Email body
    $mail->Body = '<div style="background-color: #F6F6F6;  padding:15px; font-family: "Helvetica Neue",Helvetica,Arial,sans-serif;box-sizing: border-box;">
        ...
        margin: 0 auto;">
            <div style=" padding:15px; background-color:#FFFFFF;">
                <div style="margin-bottom: 15px;">Для восстановление доступа  щелкните ссылку ниже и создайте новый пароль.</div>
                <a style="margin-bottom: 15px; display:block;" href="http://newworldinfo/index.php?&AdressMain=authorization&passforgot=' . $token . '">Восстановление доступа</a>
...
<script type="text/javascript">
    location = "/index.php?&AdressMain=authorization&forgot=yes&exist=<? echo ($exist); ?>";
</script>